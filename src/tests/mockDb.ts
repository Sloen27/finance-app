...
export const mockDb = {
  ...
  regularPayment: {
    findMany: vi.fn(async () => mockData.regularPayments),
    findUnique: vi.fn(async ({ where }) => mockData.regularPayments.find(p => p.id === where.id) || null),
    create: vi.fn(async ({ data }) => {
    ...
    update: vi.fn(async ({ where, data }) => {
      ...
      return mockData.regularPayments[index]
    }),
    delete: vi.fn(async ({ where }) => {
      const index = mockData.regularPayments.findIndex(p => p.id === where.id)
